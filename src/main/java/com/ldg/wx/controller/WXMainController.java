...
import com.ldg.wx.bo.message.Text;
...
public class WXMainController {
    ...
    public void validate(ValidateMSG validateMSG, HttpServletResponse response)
    ...
    public void message(ServerSendMSG ssdMSG, HttpServletRequest request,HttpServletResponse response)
        ...
        String AgentID=WX_Util.getXMLCDATA(message, WeixinConstant.IN_COMM_AgentID,true);
        ...
        switch (msgType) {
            case WeixinConstant.WX_MESSAGETYPE_TEXT:
                ...
                Text tx = new Text();
                tx.setAgentid(Integer.valueOf(AgentID));
                tx.setTouser(wxtext.getFromUserName());
                tx.getText().setContent(wxtext.getContent());
                String result = HttpClientUtil.getInstance().sendHttpPostJson(sendMSGRUL,  JsonUtil.parseToJson(tx));
                WX_Util.print200(response);
                break;
